...
public class Tema_Model {
    public static List<Tema> obtenerTemas(){
        ...
        try {
            try (ResultSet data = DBConecction.getData("SELECT * FROM Tema;")) {
                while(data.next()){
        ...
        try {
            ...
            try (ResultSet data = insertarCategoria.executeQuery()) {
                while(data.next()){
                    ...
                    if(relaciones){
                        PreparedStatement obtenerCategoriaLibro = DBConecction.getStatement("SELECT dLT.idLibro FROM Tema t INNER JOIN Detalle_LibroTema dLT ON t.idTema = dLT.idTema WHERE t.idTema = ?;");
    ...
    public static boolean insertar(Tema _t){
        PreparedStatement insertarSQL = DBConecction.getStatement("INSERT INTO Tema(descripcion) VALUES(?);");
        try{
    ...
    public static boolean modificar(Tema _t){
        PreparedStatement modificarSQL = DBConecction.getStatement("UPDATE Tema SET descripcion = ? WHERE idTema = ?;");
        try{
    ...
    public static boolean eliminar(Tema _t){
        PreparedStatement eliminarSQL = DBConecction.getStatement("DELETE FROM Tema WHERE idTema = ?;");
        try{
